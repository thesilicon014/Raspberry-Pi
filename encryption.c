...
int main() {
	...
	state_array[2] = state_array[2]^expandedkey[2];
	state_array[3] = state_array[3]^expandedkey[3];
	
	for(round=1; round<14; round++) {
		oper1 = state_array[0] >> 24;
		oper2 = state_array[1] >> 16;
		oper3 = state_array[2] >> 8;
		oper4 = state_array[3];
		oper5 = state_array[1] >> 24;
		oper6 = state_array[2] >> 16;
		oper7 = state_array[3] >> 8;
		oper8 = state_array[0];
		oper9 = state_array[2] >> 24;
		oper10 = state_array[3] >> 16;
		oper11 = state_array[0] >> 8;
		oper12 = state_array[1];
		oper13 = state_array[3] >> 24;
		oper14 = state_array[0] >> 16;
		oper15 = state_array[1] >> 8;
		oper16 = state_array[2];
		
		state_array[0] = t0[oper1] ^ t1[oper2] ^ t2[oper3] ^ t3[oper4] ^ expandedkey[4*round];
		state_array[1] = t0[oper5] ^ t1[oper6] ^ t2[oper7] ^ t3[oper8] ^ expandedkey[4*round + 1];
		state_array[2] = t0[oper9] ^ t1[oper10] ^ t2[oper11] ^ t3[oper12] ^ expandedkey[4*round + 2];
		state_array[3] = t0[oper13] ^ t1[oper14] ^ t2[oper15] ^ t3[oper16] ^ expandedkey[4*round + 3];
	}
	
	//Round 14
	for(oper1=0; oper1<4; oper1++) {
		temp = state_array[oper1]; // Data needed for block 5
		tempsa = temp >> 24;
		tempsb = temp >> 16;
		tempsc = temp >> 8;
		
		state_array[oper1] = ((((uint32_t)s[tempsa])<<24) ^ (((uint32_t)s[tempsb])<<16) ^ (((uint32_t)s[tempsc])<<8) ^ (uint32_t)s[((uint8_t)temp)]);
	}
	...
	state_array[0] = (((uint32_t)oper1) << 24) ^ (((uint32_t)oper2) << 16) ^ (((uint32_t)oper3) << 8) ^ ((uint32_t)oper4);
	state_array[1] = (((uint32_t)oper5) << 24) ^ (((uint32_t)oper6) << 16) ^ (((uint32_t)oper7) << 8) ^ ((uint32_t)oper8);
	state_array[2] = (((uint32_t)oper9) << 24) ^ (((uint32_t)oper10) << 16) ^ (((uint32_t)oper11) << 8) ^ ((uint32_t)oper12);
	state_array[3] = (((uint32_t)oper13) << 24) ^ (((uint32_t)oper14) << 16) ^ (((uint32_t)oper15) << 8) ^ ((uint32_t)oper16);
	
	state_array[0] = state_array[0] ^ expandedkey[56];
	state_array[1] = state_array[1] ^ expandedkey[57];
	state_array[2] = state_array[2] ^ expandedkey[58];
	state_array[3] = state_array[3] ^ expandedkey[59];
	
	printf("\n\n");
	for(oper1=0; oper1<4;oper1++){
		printf("\n    0x%08X", state_array[oper1]);
	}
	printf("\n\n");
